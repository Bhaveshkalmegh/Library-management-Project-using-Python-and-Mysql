...
def deletebook():
    ...
    paswrd = Label(win, text='ADMIN PASSWORD')
    global e1
    e1 = Entry(win)
    global e2, b2
    e2 = Entry(win)
...
def deletebooks():  # to delete a book entirely from data base id-> serial no and pass -> stud
    connectdb()
    if e2.get() == 'Bhavesh@2002':
...
def loginadmin():
    if e1.get() == 'Bhavesh kalmegh' and e2.get() == 'Bhavesh@2002':
        admin();
...
def connectdb():
    ...
    con = p.connect(host="localhost", user="root", passwd="Bhavesh@2002")
    cur = con.cursor()
...
def home():
    try:
        ...
        window = Tk()
        window.title('Sare Foundation Library Management ')# name on the login title bar
        ...
        b1 = Button(window, text=' LOGIN AS  LIBRARY MEMBER', height=2, width=23, command=loginlibr)
        b2 = Button(window, text=' LOGIN AS LIBRARIAN (ADMIN) ', height=2, width=23, command=loginadmin)
...
"""
Here there is a to role 
             1. Admin which decide to whom the access of system should be given 
             2. libraian is the one who is going to manage the library by 
                        * adding book 
                        * record issue book
                        * record return book 
                        and other activity  
                        
            admin ------> bhavesh  (only 1) 
            user -------> all  member of library  the can issue the book available in NGO 
                          as well as donate the book for the ngo  (only 39) 
